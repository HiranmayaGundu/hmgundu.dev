...
<Head title={metadata.title} description={metadata.summary} image="raft.png" />
...
<Callout>
  All requests that are sent always include the term number. If the term number
  is lower than what the server has, then that request is considered stale and
  is rejected. The server that sends the request immediately gives up its
  candidate / leader state and then updates its term number.
</Callout>
